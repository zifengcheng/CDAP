...
class FewDataSet(data.Dataset):
    ...
    def get_seq_tags(self, seq_labels):
        all_label = set()
        for seq_label in seq_labels:
            for l in seq_label:
                if l[2:] not in all_label and l != 'O':
                    all_label.add(l[2:])
        all_label = list(all_label)
        all_label.insert(0,'O')
        return all_label
    
    def get_seq_tags_ner(self, seq_labels):
        all_label = set()
        for seq_label in seq_labels:
            for l in seq_label:
                if l not in all_label and l != 'O':
                    all_label.add(l)
        all_label = list(all_label)
        all_label.insert(0,'O')
        return all_label
    ...
    def __populate__(self, samples, savelabeldic=False):
        ...
        for word, seq_tag in zip(words, seq_tags):
            ...
            for idx, (w, st) in enumerate(zip(word, seq_tag)):
                ...
                not_expand.append(1)
                for _ in word_piece[1:]:  # for sub-words
                    not_expand.append(0)
            ...
            if self.FewNERD:
                seq_tag_expand = [self.seq2label[i] for i in seq_tag_expand]
                seq_tag_expand = torch.tensor(seq_tag_expand).long()
            else:
                seq_tag_expand = [self.seq2label[i[2:]] if i!='O' else 0 for i in seq_tag_expand]
                seq_tag_expand = torch.tensor(seq_tag_expand).long()
    ...
    def __getitem__(self, index):
        ...
        if self.FewNERD:
            ...
            seq_tags = ['O'] + self.datas[index]['types']
        else:
            distinct_tags = self.__snips_get_tags__(support['seq_outs'])
            seq_tags =  self.get_seq_tags(support['seq_outs'])
        ...
        self.seq2label = {tag: idx for idx, tag in enumerate(seq_tags)}
        self.label2seq = {idx: tag for idx, tag in enumerate(seq_tags)}
