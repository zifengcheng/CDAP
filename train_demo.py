...
def main():
    ...
    parser.add_argument('--bert_path', default='/cfs/cfs-fwcdwxrr/zifengcheng/bert', type=str,
                        help='bert-path')
    ...
    if opt.dataset == 'fewnerd':
        prefix += f'-N_{opt.N}-K_{opt.K}-mode_{opt.mode}-drop_{opt.dropout}-lr_{opt.lr}-bertlr_{opt.bert_lr}-hidsize_{opt.hidsize}-graditer_{opt.grad_iter}-es_{opt.early_stop}-warmup_{opt.warmup_step}-eptasks_{opt.eposide_tasks}-weight_{opt.weight}-hyper_{opt.hyper}_{opt.consistent_loss}_{opt.temperature}_{opt.beta}'
    else:
        prefix += f'-K_{opt.K}-mode_{opt.mode}-drop_{opt.dropout}-lr_{opt.lr}-bertlr_{opt.bert_lr}-hidsize_{opt.hidsize}-graditer_{opt.grad_iter}-es_{opt.early_stop}-warmup_{opt.warmup_step}-eptasks_{opt.eposide_tasks}-weight_{opt.weight}-hyper_{opt.hyper}_{opt.consistent_loss}_{opt.temperature}'
    if opt.shuffle:
        prefix += '-sff'
    ...
    # test
    res = framework.eval(model, ckpt=ckpt,L=opt.L,dataset=opt.dataset)
    if not os.path.exists('./results'):
        os.mkdir('results')
    if  opt.dataset == 'snips':
        result_path = 'results/{}_{}_K{}_weight{}_hyper{}_beta{}_result.txt'.format(opt.dataset, opt.mode, opt.K,opt.weight,opt.hyper,opt.beta)
    else:
        result_path = 'results/{}_{}_N{}_K{}_weight{}_hyper{}_beta{}_result.txt'.format(opt.dataset, opt.mode, opt.N, opt.K,opt.weight,opt.hyper,opt.beta)
